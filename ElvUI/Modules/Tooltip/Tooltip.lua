...
local function GetItemLevelColor(ilvl)
    ...
    local q
    if ilvl < 187 then       q = 2
    elseif ilvl < 200 then   q = 3
    elseif ilvl < 226 then   q = 4
    else                     q = 5
    end
...
function TT:GetItemLvL(unit, guid)
    ...
    if ilvl and ilvl ~= -1 then
        ...
        if color then
            return string.format("%s%d|r", E:RGBToHex(color.r, color.g, color.b), ilvl)
        else
...
function TT:Initialize()
    ...
    if SocialToastFrame then
        ...
        TT:SecureHook(SocialToastFrame, "ShowToast", function(self)
